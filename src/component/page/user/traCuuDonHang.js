...
class UserCart extends Component {
  ...
  showCartShopAdmin = () => {
    ...
    let { traCuu, traCuuSDT } = this.state;
    //let { traCuuSDT } = this.state;
    //let arrTem1 = cartShopAdmin?.filter((item) => item.data.MDH === traCuu);
    let arrTem = cartShopAdmin?.filter(
      (item) => item.data.user.phone === traCuuSDT
    );
    let arrTem1 = cartShopAdmin?.filter((item) => item.data.MDH === traCuu);
    let arrTemp2 = [...arrTem, ...arrTem1];
    let arrTemp3 = [...new Set(arrTemp2)];
    let resule = "";
    resule = arrTemp3?.map((item, i) => {
      return (
        <div
